...
Continue: Fix mouse input in spudstorm. Read src/scenes/Game.ts and change `mouseActive` to initialize as `true` instead of `false`. Remove the debugText overlay. Then run `npm run build`, commit src/main.ts and src/scenes/Game.ts with message "fix: initialize mouseActive true, remove debug overlay", and push to GitHub. Cloudflare auto-deploys to https://spudstorm.pages.dev. Project is at /home/shazbot/Projects/products/games/spudstorm/. Read RESUME_SPUDSTORM.md for full context.
