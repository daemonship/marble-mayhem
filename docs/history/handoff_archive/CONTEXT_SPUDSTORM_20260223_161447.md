...
Continue: Fix mouse input in spudstorm game at https://spudstorm.pages.dev. Debug text overlay is live on the canvas (yellow text at bottom showing "ptr: X,Y  active:true/false  player: X,Y"). Ask the user what those values show when they move the mouse over the game canvas after clicking Start. If mouseActive never becomes true, set it to true at game start unconditionally (user just clicked Start so they have a mouse) by initializing `private mouseActive: boolean = true` in Game.ts instead of relying on the pointermove event. After fixing: remove the debugText, commit all uncommitted changes in src/main.ts and src/scenes/Game.ts, push to GitHub, and Cloudflare auto-deploys. Read RESUME_SPUDSTORM.md in /home/shazbot/Projects/products/games/spudstorm/ for full context.
